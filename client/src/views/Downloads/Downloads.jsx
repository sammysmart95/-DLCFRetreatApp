    ...
    return (
      ...
      <BlockUI blocking={blocking} >
        <Card>
          ...
          <CardBody>
            { !files.length && blocking ? <h3> Searching </h3> : !files.length && !blocking ?
            ...
            <Table bordered striped hover>
              ...
              <tbody>
                {this.state.files.map(downloadFile => (
                  <tr key={downloadFile._id} >
                    <th scope="row">1</th>
                    <td>{downloadFile.fileName}</td>
                    <td onClick={e=>this.downloadFile(downloadFile._id, `${downloadFile.originalName}`)}>
